...
#                     ux_nday = datetime2ts(mid)
    ...
    #                 wtag += ' %s=%s' % ('season',season)
                    wtag += ' \n'
                    _query_result = query(start, end, __m , qtag)
...
#                     denom2 = 86400 # 60 * 60 * 24 = 하루를 second로 표시
                    ...
                    if len(_query_result[0]['dps']) and tot_v == 0:continue
                    calc_v += (tot_v / len(_query_result[0]['dps']))
                    tot_v = 0.0
                calc_v = calc_v / 365
                sockWriteTSD( sock, __wm, 1467298800, calc_v, wtag)
...
def run():
    ...
    __wm = 'rc06_ld_rate_daily_v1_testtest'       # put할 metric명
    __v = None

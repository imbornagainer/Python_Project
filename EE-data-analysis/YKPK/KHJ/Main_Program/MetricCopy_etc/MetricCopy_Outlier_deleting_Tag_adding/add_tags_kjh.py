...
tag_ref = {
    ...
    u'백화점': 'departmentStore', u'섬유': 'fiber', u'학교': 'school', u'식품': 'food', u'화공': 'fire',
...
#         print 'tags : %s' % dp['tags']['modem_num']
#         print 'tags _mds_id : %s' % dp['tags']['_mds_id']
#         print 'timestamp : %s' % dp['timestamp']
#         print 'value : %s' % dp['value']
        if (dp['value'] < 101.0):
            if len(send) >= 1024:
                #print send + '\n\n'
                sock.sendall(send)
...
if __name__ == '__main__':
    ...
    last  = '2016/07/02-00:00:00'
    metric_in  = 'none:rc04_simple_data_v3'
    chk_number = 0
    #metric_in  = 'sum:rc04_simple_data_v3'
    metric_out = 'rc05_excel_detail_copy_testtest_tag_v2'
    #metric_out = 'RazaTheChained_v4'
    out_file = open(metric_out, 'w')
    led_list = []
    inv_list = []

    wb = load_workbook(filename='ref_list_detail2.xlsx', read_only=True)
    ws = wb[u'최종(LED+인버터)']
...
#     print 'led_list : %s' % led_list
#     print 'inv_list : %s' % inv_list

...
# for modem문 주석추가
# 지정된 날짜구간에서 load에 대한 가동률을 확인하기 위한 source
...
def tsdb_query(query, start, end, metric, modem_num, holiday, load_type):
    param = {}
    if start: param['start'] = start
    if end: param['end'] = end
    param['m'] = metric + '{' + 'holiday={0!s},modem_num={1!s},load={2!s}'.format(holiday, modem_num, load_type) + '}'
    #print param
    if 1:
...
if __name__ == '__main__':
    ...
    metric_out = 'rc05_BRR_15min_fan_v1'                            # put할 metric명
    outlier_txt_name = '__171114_brc_fan_001.txt'                   # outlier log를 저장할 txt명
    load_type = 'fan'                                               # get할 load명
...
#         if __done > 5 : exit()
        ...
        for modem in modem_list:
            ...
            query_result = tsdb_query(query, start, add_time(end, seconds=1), metric_in, modem, modem_holiday, load_type)

...
def Chk_Outlier(chk_num, adj_modem_num,mds_id,unix_time, value):
    if chk_num == 1:
        f = open('outlier_lists_171102_v1.txt', 'w')
        f.write('%d:\n' % chk_num)
        f.write('modem_num : %s\n' % adj_modem_num)
        f.write('_mds_id   : %s\n' % mds_id)
        ...
        f = open('outlier_lists_171102_v1.txt', 'a')
        f.write('%d:\n' % chk_num)
        f.write('modem_num : %s\n' % adj_modem_num)
        f.write('_mds_id   : %s\n' % mds_id)
        f.write('unix_time : %s\n' % ts2datetime(unix_time))
...
if __name__ == '__main__':
    ...
    metric_out = 'rc05_no_outlier_led_interver_tag_v1'
...
#     wb = load_workbook(filename='ref_list_kjh.xlsx', read_only=True)
#     ws = wb[u'최종(LED+인버터)']

...
class u_ee_tsdb:
    ...
    def __init__(self, __url, __st=None, __et=None, __r=None):
        self._url(__url)
        if __st != None and __et != None and __r != None:
            exit(" < Input Error > no time info")
        elif __st != None and __et != None:
            self._set_time_period(__st, __et)
        elif __st != None:
            self._set_time_period(__st)
        elif __r != None:
            self.__cin_recent = True
        elif self.__cin_recent != True:
...
def parse_args():
    ...
    parser.add_argument("-start", default=20170701, help="start time input, like 2016110100")
    parser.add_argument("-end", default=20170731, help="end time input, like 2016110223")
...
def rtest(__url, __m, __tag, __stime):
    ...
    stime = __stime
    if (tag == None): return
    data = tsdbclass.readTSDrecent(tag, 'none', stime)
    return data
...
if __name__ == "__main__":
    ...
    new_metric = 'rc05_add_tag_v4'

    check_num = 0
    for model_num in modem_list:
        check_num += 1
        adj_model_num = str(model_num)[2:-2]
        print (check_num, adj_model_num)
        tag = {'modem_num': adj_model_num}

        if recent == 'True':
            get_data = rtest(u, metric, tag, stime)
        elif (etime == None) and (stime != None):
            rt = point(u, metric, stime, tag)

        if type(get_data) == None: continue  #if no data, go to the next iteration

        for i in range(len(get_data)):
            mds_id = get_data[i]['tags']['_mds_id']
            holiday = get_data[i]['tags']['holiday']

            if model_num in led_modem_list: led_inverter = 'led'
            elif model_num in inverter_modem_list: led_inverter = 'inverter'
            else: led_inverter = 'other'

            for unix_time, value in get_data[i]['dps'].items():
                input_data = {
                    "metric": new_metric,
                    "timestamp": unix_time,
                    "value": value,
        ...
        # python useTSDB_1011.py -url tinyos.iptime.org -port 4242 -recent True -m rc01.t_power.WH

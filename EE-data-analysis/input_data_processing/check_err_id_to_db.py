...
# file input id list, it should be prepared before start
# please read __sample_id_list.py file in the same directory
import __inverter_err_id_input_list
import __led_err_id_input_list
import result_ids
...
def go():
        ...
        # 디렉토리에 아래 이름의 파일이 있어야 함
        # 파일 안에는 err_id_list 리스트로 아이디가 존재해야함

        if (_t_type_ == 'inverter'):
            _input_ = __inverter_err_id_input_list.err_id_list
        elif (_t_type_ == 'led'):
            #_input_ = __led_err_id_input_list.err_id_list
            _input_ = result_ids.result_ids

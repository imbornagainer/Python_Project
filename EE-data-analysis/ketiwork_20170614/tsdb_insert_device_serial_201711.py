...
def updateMINONE(tindex, in_sttime, in_entime, in_mdsid, tagdata):
        ...
        sql_tmp = "SELECT LP_TIME, VALUE_00, VALUE_15, VALUE_30, VALUE_45, MDS_ID, DEVICE_SERIAL, WRITEDATE, CONTRACT_ID FROM EMNV_LP_EM_VIEW ELE WHERE ('%s' <= ELE.LP_TIME AND ELE.LP_TIME <= '%s') AND ELE.CHANNEL = 1 AND ELE.DEVICE_SERIAL = '%s'" %(starttime, endtime, serialid)
...
if __name__ == "__main__":
        ...
        u_tag = in_list.coolboil_list

        toendpoint = len(u_tag)

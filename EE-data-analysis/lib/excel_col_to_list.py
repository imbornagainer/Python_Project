...
def get_list( filename, start, end, sheet_num = 0):
    ...
    sh = e1.get_sheet_by_name(sheets[int(sheet_num)])
    getlist = exl_class.read_vertical(sh, start, end)

    return getlist, len(getlist)

if __name__ == "__main__":

    # 입력파일
    E1_file = u"EE_cool.xlsx"

    exl_class = excell_class()
    e1 = exl_class.open_exc_doc(E1_file)
    sheets = e1.get_sheet_names()
    sh1 = e1.get_sheet_by_name(sheets[1])
    led_list = exl_class.read_vertical(sh1,'K2','K96')
    #inverter_list = exl_class.read_vertical(sh1,'AC4','AC539')

    print led_list
    #print inverter_list
    print "led", len(led_list)
    #print "inverter", len(inverter_list)

...
if __name__ == '__main__':
    ...
    opts = {'-i' : 'rc06_none_dps_v00', '-o' : 'rc06_total_cnt__001', '-r' : 'count_list_new.json', '-e' : '1', '-s' : '2016/07/01', '-z' : '2017/07/01', '-d' : '', '-q' : ''}
            ...
            for tag in tag_list[key]: # tag --> building.office etc
                ...
                for detail in tag_list[key][tag]:
                    ...
                    while tsdb.is_past(end, last):
                        start = end
                        end = tsdb.add_time(start, days=int(duration))
                        device_list = []
                        result_dps  = {}
                        ...
                        query_result = tsdb.query(start, tsdb.add_time(end, days=1), metric_in, query_tags) # none dps 갯수 읽기

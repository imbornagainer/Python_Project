...
def parse_args():
    ...
    parser.add_argument("-wm", default=None, help="write-metric ")
...
def cpmetric(__url, __m,  __st, __et, __wm):
    ...
    #tsdb에서 데이터 읽어오기
    _buf_dict = tsdbclass.readTSD()
    #print _buf_dict['metric']
    #print _buf_dict['dps']
    #print _buf_dict['dps'].keys()
    for (k, v) in _buf_dict['dps'].items(): 
        #pair = '%s=%s' % (k, v)
        __ut = '%s' %(k)
        __v = '%s' %(v)
        sockWriteTSD( __wm, __ut, __v ) 
    print  'write done' 

def q_cpmetric(__url, __m,  __st, __et, __wm):
    tsdbclass = u_ee_tsdb(__url, __st, __et)
    tsdbclass.set_metric(__m)
...
if __name__ == "__main__":
    ...
    q_cpmetric(u, metric, stime, etime, write_metric )
